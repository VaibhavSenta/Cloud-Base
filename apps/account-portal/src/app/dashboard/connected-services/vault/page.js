...
export default function VaultSettingsPage() {
  ...
  const disconnectMutation = useMutation({
    mutationFn: async () => {
      const res = await api.post('/auth/connected-services/disconnect', { serviceId: 'vault' });
      return res.data;
    },
    onSuccess: async () => {
      ...
      if (userCache) {
        ...
        if (decryptedUser) {
          ...
          decryptedUser.connectedServices = decryptedUser.connectedServices.filter(s => s.serviceId !== 'vault');
  ...
  const isConnected = user?.connectedServices?.some(cs => cs.serviceId === 'vault');
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '8px' }}>Vault Not Linked</h2>
          <p style={{ fontSize: '0.82rem', color: '#888888', lineHeight: 1.4, marginBottom: '24px' }}>
            Cloud Vault settings are only configurable once you authorize the service connection.
          </p>
          <button 
            onClick={() => router.push('/dashboard/connected-services/explore/vault')}
            style={{
              display: 'block',
              width: '100%',
              background: '#0095f6',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        >
          {disconnectMutation.isPending ? 'Disconnecting...' : 'Disconnect Vault'}
        </button>
