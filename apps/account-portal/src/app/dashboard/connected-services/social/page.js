...
export default function SocialSettingsPage() {
  ...
  const disconnectMutation = useMutation({
    mutationFn: async () => {
      const res = await api.post('/auth/connected-services/disconnect', { serviceId: 'social' });
      return res.data;
    },
    onSuccess: async () => {
      ...
      if (userCache) {
        ...
        if (decryptedUser) {
          ...
          decryptedUser.connectedServices = decryptedUser.connectedServices.filter(s => s.serviceId !== 'social');
  ...
  const isConnected = user?.connectedServices?.some(cs => cs.serviceId === 'social');
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '8px' }}>Social Not Linked</h2>
          <p style={{ fontSize: '0.82rem', color: '#888888', lineHeight: 1.4, marginBottom: '24px' }}>
            Cloud Social settings are only configurable once you authorize the service connection.
          </p>
          <button 
            onClick={() => router.push('/dashboard/connected-services/explore/social')}
            style={{
              display: 'block',
              width: '100%',
              background: '#0095f6',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '8px' }}>Cloud Social Settings</h2>
        <p style={{ fontSize: '0.82rem', color: '#888888', lineHeight: 1.4, marginBottom: '24px' }}>
          Configure profile metadata, fediverse connections, and privacy bounds for Cloud Social.
        </p>
        ...
        >
          {disconnectMutation.isPending ? 'Disconnecting...' : 'Disconnect Social'}
        </button>
